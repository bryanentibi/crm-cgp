...
def is_mobile(t):
    c = convert_tel(t)
    return c.startswith('06') or c.startswith('07')

def is_tel(t):
    return bool(re.match(r'^0033\d{8,}$', str(t)))
        ...
        for ds in dsr.get('DS', []):
            for row in ds.get('PH', []):
                for item in row.get('DM0', []):
                    ...
                    # Trier fixes et mobiles
                    for t in tels:
                        if is_mobile(t):
                            tel_port = convert_tel(t)
                        else:
                            tel_fixe = convert_tel(t)
...
for ex in avec_port[:5]:
    print(f"  {ex['nom']} {ex['prenom']} -> {ex['telephone_direct']} ({ex['ville']})")
