...
2. Enrichissement INSEE dates de création
"""
...
VILLES = [
    ...
    ("Montpellier", 43.6108, 3.8767), ("Bordeaux", 44.8378, -0.5792),
...
def scrape_google_places():
    ...
    # Récupérer les tels déjà en base
...
def search_insee(nom, prenom='', cp=''):
    ...
    try:
        ...
        nom_upper = nom.upper()
        prenom_upper = prenom.upper() if prenom else ''
        
        for res in results:
            nom_res = (res.get('nom_complet', '') or '').upper()
            if nom_upper not in nom_res:
                if prenom_upper and prenom_upper not in nom_res:
                    continue
...
def main():
    ...
    # 1. Scraping Google Places
    n_artisans = scrape_google_places()
    
    # 2. Enrichissement INSEE
    n_insee = enrichir_insee()
    
    print("\n" + "="*60)
    print(f"🎉 TERMINÉ: +{n_artisans} artisans | {n_insee} contacts enrichis INSEE")
    print("="*60)
